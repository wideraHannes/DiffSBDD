...
from equivariant_diffusion.conditional_model import (
    ConditionalDDPM,
...
class LigandPocketDDPM(pl.LightningModule):
    def __init__(
        ...
        self.eval_batch_size = (
            eval_params.eval_batch_size
            if "eval_batch_size" in eval_params
            else batch_size
        )
        self.lr = lr
    ...
    def forward(self, data):
        ...
        if self.loss_type == "l2" and self.training:
            actual_ligand_size = (
                ligand["size"] - ligand["num_virtual_atoms"]
                if self.virtual_nodes
                else ligand["size"]
            )
            ...
            # normalize loss_t
            denom_lig = (
                self.x_dims * actual_ligand_size + self.ddpm.atom_nf * ligand["size"]
            )
        ...
        if self.auxiliary_loss and self.loss_type == "l2" and self.training:
            ...
            weighted_lj_potential = self.auxiliary_weight_schedule(
                t_int.long()
    ...
    def _shared_eval(self, data, prefix, *args):
        ...
        self.log_metrics(
            info, prefix, batch_size=len(data["num_lig_atoms"]), sync_dist=True
        )
    ...
    def sample_and_analyze(self, n_samples, dataset=None, batch_size=None):
        ...
        for i in range(math.ceil(n_samples / batch_size)):
            ...
            num_nodes_lig, num_nodes_pocket = self.ddpm.size_distribution.sample(
                n_samples_batch
            )

            xh_lig, xh_pocket, lig_mask, _ = self.ddpm.sample(
                n_samples_batch, num_nodes_lig, num_nodes_pocket, device=self.device
            )
    ...
    def analyze_sample(self, molecules, atom_types, aa_types, receptors=None):
        ...
        kl_div_atom = (
            self.ligand_type_distribution.kl_divergence(atom_types)
            if self.ligand_type_distribution is not None
            else -1
        )
        kl_div_aa = (
            self.pocket_type_distribution.kl_divergence(aa_types)
            if self.pocket_type_distribution is not None
            else -1
        )
        ...
        qed, sa, logp, lipinski, diversity = self.molecule_properties.evaluate_mean(
            connected_mols
        )
    ...
    def sample_and_analyze_given_pocket(self, n_samples, dataset=None, batch_size=None):
        print(
            f"Analyzing sampled molecules given pockets at epoch "
        ...
        return self.analyze_sample(molecules, atom_types, aa_types, receptors=receptors)

    def sample_and_save(self, n_samples):
        num_nodes_lig, num_nodes_pocket = self.ddpm.size_distribution.sample(n_samples)

        xh_lig, xh_pocket, lig_mask, pocket_mask = self.ddpm.sample(
            n_samples, num_nodes_lig, num_nodes_pocket, device=self.device
        )
    ...
    def sample_and_save_given_pocket(self, n_samples):
        batch = self.val_dataset.collate_fn(
            [
                self.val_dataset[i]
                for i in torch.randint(len(self.val_dataset), size=(n_samples,))
            ]
        )
        ligand, pocket = self.get_ligand_and_pocket(batch)
    ...
    def sample_chain_and_save(self, keep_frames):
        ...
        one_hot_lig = F.one_hot(
            torch.argmax(one_hot_lig, dim=2), num_classes=len(self.lig_type_decoder)
        )
    ...
    def prepare_pocket(self, biopython_residues, repeats=1):
        if self.pocket_representation == "CA":
            pocket_coord = torch.tensor(
                np.array([res["CA"].get_coord() for res in biopython_residues]),
                device=self.device,
                dtype=FLOAT_TYPE,
            )
            ...
            pocket_atoms = [
                ...
                if (
                    a.element.capitalize() in self.pocket_type_encoder
                    or a.element != "H"
                )
            ]
            pocket_coord = torch.tensor(
        ...
        pocket_size = torch.tensor(
            [len(pocket_coord)] * repeats, device=self.device, dtype=INT_TYPE
        )
        pocket_mask = torch.repeat_interleave(
            torch.arange(repeats, device=self.device, dtype=INT_TYPE), len(pocket_coord)
        )
    ...
    def generate_ligands(
        ...
        **kwargs,
        ...
        if type(self.ddpm) == EnVariationalDiffusion:
            ...
            ligand = {
                "x": torch.zeros(
                    (len(lig_mask), self.x_dims), device=self.device, dtype=FLOAT_TYPE
                ),
                "one_hot": torch.zeros(
                    (len(lig_mask), self.atom_nf), device=self.device, dtype=FLOAT_TYPE
                ),
                "size": num_nodes_lig,
                "mask": lig_mask,
            }
            ...
            pocket_mask_fixed = torch.ones(len(pocket["mask"]), device=self.device)
        ...
        molecules = []
        for mol_pc in zip(
            utils.batch_to_list(x, lig_mask), utils.batch_to_list(atom_type, lig_mask)
        ):
