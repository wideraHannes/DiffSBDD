...
class ConditionalDDPM(EnVariationalDiffusion):
    ...
    def sample_normal_zero_com(
        self, mu_lig, xh0_pocket, sigma, lig_mask, pocket_mask, fix_noise=False
    ):
    ...
    def partially_noised_ligand(self, ligand, pocket, noising_steps):
        ...
        t_int = (
            torch.ones(
                size=(ligand["size"].size(0), 1), device=ligand["x"].device
            ).float()
            * noising_steps
        )
    ...
    def diversify(self, ligand, pocket, noising_steps):
        ...
        z_lig, xh_pocket, _ = self.partially_noised_ligand(
            ligand, pocket, noising_steps
        )
        ...
        x_lig, h_lig, x_pocket, h_pocket = self.sample_p_xh_given_z0(
            z_lig, xh_pocket, lig_mask, pocket["mask"], n_samples, pocket_emb=pocket_emb
        )
