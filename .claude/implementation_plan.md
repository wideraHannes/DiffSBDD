...
- [ ] **1.1 ESM-C Infrastructure**
  ...
  - [ ] from esm.sdk import client model = client(
        model="esmc-300m-2024-12", url="https://forge.evolutionaryscale.ai", token=token
        )
  - [ ] Test ESM-C on single protein
  - [ ] Extract embedding for 1 test pocket
  - [ ] Verify embedding shape (960,) and statistics
  - [ ] Create extraction script template

- [ ] **1.2 Code Integration**
  - [ ] Modify `dataset.py` - add esmc_path parameter
  - [ ] Modify `dynamics.py` - add FiLM network
  - [ ] Update all 8 dynamics() calls in codebase
  - [ ] Create ESM-C config file
